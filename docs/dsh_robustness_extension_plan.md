...
| The model may be measuring work/topic rather than author. | **E2 infrastructure added:** leave-one-work-out validation now has a locked protocol, runnable script, checker, and workflow; outputs still need to be generated by workflow. |
...
| E2 | Essential | Held-out-work validation | **Infrastructure complete; workflow output pending** | Yes | `docs/heldout_work_validation_protocol.md`, `scripts/23_run_heldout_work_validation.py`, `scripts/check_e2_heldout_work_validation.py`, `.github/workflows/e2_heldout_work_validation.yml`, planned `metadata/heldout_work_fold_registry.csv`, `data/results/heldout_work_transfer_predictions.csv`, `metadata/heldout_work_transfer_summary.csv`, `metadata/heldout_work_same_condition_summary.csv`, `metadata/heldout_work_distance_summary.csv`, `metadata/heldout_work_manifest.csv`, `logs/heldout_work_validation_report.md` |
...
Run the fresh E2 workflow:

```text
Actions → E2 Held-Out-Work Validation Fresh Run → Run workflow
```

After it commits outputs, inspect `logs/e2_heldout_work_validation_workflow_proof.md` and the E2 summary tables before making any manuscript claim.

## Execution order

1. **E3 warning-row sensitivity is complete.** Use it as the first reviewer-facing robustness result.
2. **Run E2 held-out-work validation.** This is now the most important methodological upgrade because it attacks the work/topic confound.
...
E2 must not be cited as completed until the workflow-generated outputs and proof log exist.